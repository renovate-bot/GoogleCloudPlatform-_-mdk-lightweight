...
"""This provides a convenience function to prepare, optimize, fit and save a
SciKitLearn model locally, without invoking a pipeline.
"""
...
def run(
    ...
    """This provides a mechanism to prepare, optimize, fit and save a
    SciKitLearn model locally, without invoking a pipeline.
    ...
    google.cloud.aiplatform.init(project=project_id, location=gcp_config.get("region"))
    ...
    # Get our data:
    data_uris = sklearn_example.prepare.prepare(
        general_config_filename, project_id, environment
    )

    # Optimize hyperparamters.
    hyperparameters = sklearn_example.optimize_hyperparameters.optimize_hyperparameters(
        general_config_filename, data_uris.val_uri, environment
    )

    # Train our model:
    model = sklearn_example.train.train(
    ...
    # Evaluate our model:
    eval = sklearn_example.evaluate.evaluate(
...
if __name__ == "__main__":
    ...
    pipeline_module_file = pipeline_mapping["sklearn_example"]
