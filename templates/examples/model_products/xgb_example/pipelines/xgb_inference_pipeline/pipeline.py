...
@kfp.dsl.pipeline(name="xgb_inference_pipeline")
def pipeline(
    environment: str,
):
    ...
    xgb_batch_predict = mdk.util.framework.loadComponentSpec(pc_filename, "xgb_batch_predict")  # fmt: skip
    ...
    predict_task = xgb_batch_predict(
        gcp_config_filename=gcp_config_filename,
        pipeline_config_filename=pc_filename,
        general_config_filename=general_config_filename,
    )
    with kfp.dsl.Condition(
        ...
        predict_task.outputs["run_monitoring"] == True,  # noqa: E712
    ...
    mdk.util.framework.apply_resource_settings_to_task(
        task_object=predict_task,
        pipeline_config_filename=pc_filename,
        component_name=predict_task.name,
    )
    mdk.util.framework.apply_resource_settings_to_task(
